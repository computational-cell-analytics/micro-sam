...
class AMGBase(ABC):
    ...
    def _postprocess_masks(self, mask_data, min_mask_region_area, box_nms_thresh, crop_nms_thresh, output_mode):
        ...
        # We require the binary mask output for creating the instance-seg output.
        elif output_mode in ("binary_mask", "instance_segmentation"):
            mask_data["segmentations"] = [amg_utils.rle_to_mask(rle) for rle in mask_data["rles"]]
        elif output_mode == "rle":
    ...
    def generate(
        ...
        Args:
            ...
            output_mode: The form masks are returned in. Possible values are:
                ...
                - 'rle': Return a list of dictionaries with run-length encoded masks.
...
class InstanceSegmentationWithDecoder:
    ...
    def _to_masks(self, segmentation, output_mode):
        if output_mode != "binary_mask":
            raise ValueError(
                f"Output mode {output_mode} is not supported. Choose one of 'instance_segmentation', 'binary_masks'"
            )
...
class AutomaticPromptGenerator(InstanceSegmentationWithDecoder):
    ...
    def generate(
        ...
        The instance segmentation is generated by deriving prompts from the foreground and
        distance predictions of the segmentation decoder by thresholding these predictions,
        intersecting them, computing connected components, and then using the component's
        centers as point prompts. The masks are then filtered via NMS and merged into a segmentation.
        ...
        Args:
            ...
            intersection_over_min: Whether to use the minimum area of the two objects or the
                intersection over union area (default) in NMS.
            output_mode: The form masks are returned in. Possible values are:
                - 'binary_mask': Return a list of dictionaries with masks encoded as binary masks.
                - 'instance_segmentation': Return masks merged into an instance segmentation in a single array.
                By default, set to 'instance_segmentation'.
    ...
    def generate(
        ...
        Args:
            ...
            optimize_memory: Whether to optimize the memory consumption by merging the per-slice
                ...
                `refine_with_box_prompts=False` and `output_mode="instance_segmentation"`.
    ...
    **kwargs,
    ...
    Args:
        ...
        segmentation_mode: The segmentation mode. One of 'amg', 'ais', or 'apg'.
            By default, '{DEFAULT_SEGMENTATION_MODE_WITH_DECODER}' is used if a decoder is passed,
            otherwise 'amg' is used.
    ...
    if segmentation_mode is None:
        segmentation_mode = "amg" if decoder is None else DEFAULT_SEGMENTATION_MODE_WITH_DECODER
