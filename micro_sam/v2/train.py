# Code adapted from SAM2: https://github.com/facebookresearch/sam2
...
def train_sam2(
    ...
    """Train Segment Anything 2 model.

    Args:
        config: Filepath to the configuration file for training SAM2 model.
        use_cluster: Whether to launch training on a cluster or run locally.
        partition: The SLURM partition.
        account: The SLURM account.
        qos: The SLURM qos.
        num_gpus: The number of GPUs per node.
    ...
    if cfg.launcher.experiment_log_dir is None:
        cfg.launcher.experiment_log_dir = os.path.join(os.getcwd(), "sam2_logs", config)
    ...
    cfg.launcher.gpus_per_node = (num_gpus if num_gpus is not None else cfg.launcher.gpus_per_node)
    cfg.launcher.num_nodes = (num_nodes if num_nodes is not None else cfg.launcher.num_nodes)
    submitit_conf.use_cluster = (use_cluster if use_cluster is not None else submitit_conf.use_cluster)
    if submitit_conf.use_cluster:
        executor = submitit.AutoExecutor(folder=submitit_dir)

        submitit_conf.partition = (partition if partition is not None else submitit_conf.get("partition", None))
        submitit_conf.account = (account if account is not None else submitit_conf.get("account", None))
        submitit_conf.qos = (qos if qos is not None else submitit_conf.get("qos", None))

        job_kwargs = {
            "timeout_min": 60 * submitit_conf.timeout_hour,
            "name": (submitit_conf.name if hasattr(submitit_conf, "name") else config),
