...
class PromptGeneratorBase:
    def __init__(
        ...
        n_positive_points: int = 1,
        n_negative_points: int = 1,
    ...
    def _get_iterative_positive_points(self, pos_region, overlap_region):
        """Function to obtain positive point prompts (coordinates and labels) for iterative prompting
        """
    ...
    def _get_iterative_negative_points(self, negative_region_batched, true_object_batched, gt_batched):
        """Function to obtain negative point prompts (coordinates and labels) for iterative prompting
        """
        ...
        for neg_region, true_object, gt in zip(negative_region_batched, true_object_batched, gt_batched):
            ...
            if torch.stack(tmp_neg_loc).shape[-1] == 0:
                ...
                bbox_mask[max(bbox[0] - self.dilation_factor, 0): min(bbox[2] + self.dilation_factor, gt.shape[-2]),
                          max(bbox[1] - self.dilation_factor, 0): min(bbox[3] + self.dilation_factor, gt.shape[-1])] = 1
    ...
    def _sample_batched_positive_points(self, object_mask, center_coordinates, coord_list, label_list):
    ...
    def _sample_batched_negative_points(self, object_mask, bbox_coordinates, coord_list, label_list):
    ...
    def _sample_points(self, object_mask, bbox_coordinates, center_coordinates):
        ...
        coord_list, label_list = self._sample_batched_positive_points(object_mask, center_coordinates,
                                                                      coord_list, label_list)
        coord_list, label_list = self._sample_batched_negative_points(object_mask, bbox_coordinates,
                                                                      coord_list, label_list)
...
class IterativePromptGenerator(PromptGeneratorBase):
    ...
    def __call__(
        ...
        pos_coordinates, pos_labels = self._get_iterative_positive_points(pos_region, overlap_region)
        neg_coordinates, neg_labels = self._get_iterative_negative_points(neg_region, true_object, gt)
