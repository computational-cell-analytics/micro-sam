...
from micro_sam.sam_annotator.annotator_tracking import AnnotatorTracking, batch_tracking_annotator


@pytest.mark.gui
@pytest.mark.skipif(platform.system() in ("Windows",), reason="Gui test is not working on windows.")
def test_batch_tracking_navigation(make_napari_viewer_proxy):
    """Drive the tracking batch harness: each item is a video, advancing saves the tracks and loads
    the next video.
    """
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        viewer = batch_tracking_annotator(
        ...
        assert "batch_next" in state.widgets
