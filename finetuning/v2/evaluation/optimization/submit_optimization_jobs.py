...
SLURM_ARRAY_TASK_ID, retries in-process failures, and records the outcome of every task as a
...
The presets encode the cluster facts of grete: '2d' runs on a 10 GB MIG slice, '3d' on a 20 GB one.
Canonical timing trials must share one hardware identity, so run them with '--throttle 1' on a
fixed GRES type.
"""
...
def render_job_script(
    ...
    header = [
        ...
        f"#SBATCH --array=0-{n_tasks - 1}%{throttle}",
...
if [ $rc -eq 0 ]; then
  ...
  printf 'exit=%s elapsed=%s attempts=%s job=%s\\n' "$rc" "$elapsed" "$attempt" "$SLURM_JOB_ID" > "$markers/$tag.failed"
fi
exit $rc
"""
    return "\n".join(header) + "\n" + body
...
def write_job_dir(
    ...
    script = render_job_script(name, job_dir, len(tasks), resources, throttle, dependency, attempts)
...
def submit_tasks(
    ...
    if not tasks:
        print("Nothing to do.")
        return None, None
    job_dir = write_job_dir(name, tasks, resources, jobs_root, throttle, dependency, attempts, argv)
    print(f"Job directory: {job_dir} ({len(tasks)} tasks)")
    if dry_run:
    ...
    if job_id_path.exists():
        for row in _sacct_rows(job_id_path.read_text().strip()):
            for index in _expand_array_ids(row["JobID"]):
                states[index] = row
    failing = 0
    name = sanitize(json.loads((job_dir / "submit.json").read_text())["name"])
    for index, (tag, _) in enumerate(tasks):
        row = states.get(index, {})
