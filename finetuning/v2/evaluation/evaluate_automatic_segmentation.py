...
def tune_parameters(
    ...
    The sweep and the evaluation load their samples through the same `common.load_data`, so the
    parameters are selected on data that is preprocessed exactly like the data they are scored on.
    ...
    Args:
        model: The model of the mode, from `build_model`.
        mode: The segmentation mode, 'ais' or 'apg'.
        dataset_name: The dataset to tune on.
...
def main():
    ...
    parser.add_argument("--n_tuning_samples", type=int, default=None, help="Cap the sweep to this many samples.")
