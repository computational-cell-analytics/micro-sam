...
5. The menu for automatic segmentation. Click `Automatic Segmentation` to segment all objects in the image. The results appear in the `auto_segmentation` layer. Models without a decoder use automatic mask generation (AMG). Models with a decoder offer sparse and dense instance segmentation and automatic prompt generation (APG). Select the method in the `mode` menu. The advanced settings show the parameters for the selected method.
